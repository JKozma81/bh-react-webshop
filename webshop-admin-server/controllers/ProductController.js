...
class ProductController {
      ...
      try {
        ...
        form.parse(req, async (err, fields, files) => {
          ...
          if (desc.length >= 240) {
            throw {
              errors: [
                {
                  type: 'validation',
                  message: 'Description must be below 240 character long!',
                },
              ],
              productData: { ...fields },
            };
          ...
          if (sku.length > 12) {
            throw {
              errors: [
                {
                  type: 'validation',
                  message: 'SKU must be below 12 character long!',
                },
              ],
              productData: { ...fields },
            };
          ...
          try {
            ...
            if (!sku || sku === 'undefined') {
              fs.mkdirSync(`./static/img/${newSku.toUpperCase()}`);
              imageSrc += newSku;
            } else {
  ...
  static getProductsAndImages(options) {
    const productService = options.productService;
    const imageService = options.imageService;
    const promotionsService = options.promotionsService;
    const recommendationsService = options.recommendationsService;

    return async (req, res) => {
      try {
        const productsData = await productService.getAllProducts();
        const imageData = await imageService.getAllImages();
        const promotions = await promotionsService.getAllPromos();
        const recommendations = await recommendationsService.getAllRecommended();
  ...
  static deleteImageFromDB(options) {
    ...
    return async (req, res) => {
      try {
        const fileInfo = await imageService.deleteImage(Number(req.params.id));
  ...
  static deleteProduct(options) {
    ...
    return async (req, res) => {
      try {
        ...
        const remainingProducts = await productService.deleteProduct(deleteSku);
        const remainingImages = await imageService.deleteAllImages(deleteSku);
  ...
  static uploadPromotions(options) {
    const promotionsService = options.promotionsService;
    return async (req, res) => {
      try {
        const form = formidable({ multiples: true });
        ...
        await form.parse(req, async (err, fields, files) => {
          ...
          const { promo_text, product_sku } = fields;

          const productSKU = product_sku.split(', ')[0];

          try {
            let imageSrc = `http://localhost:5000/img/promotions`;

            fs.copyFileSync(
              files.images.path,
              `./static/img/promotions/${files.images.name}`,
              (err) => {
                throw {
                  errors: [
                    {
            ...
            const imageUrl = imageSrc + '/' + files.images.name;

            const result = await promotionsService.addPromotion({
              productSKU,
              promo_text,
              picture: imageUrl,
            });
